...
_COMPAT_1_0 = {"mit","bsd-2-clause","bsd-3-clause","apache-2.0","isc","lgpl-2.1","lgpl-2.1-only","lgpl-2.1-or-later","mpl-2.0","cddl-1.0","cddl-1.1"}
_INCOMPAT_0_0 = {"gpl-3.0","gpl-3","agpl-3.0","agpl-3","cc-by-nc","cc-by-nc-4.0","proprietary","noncommercial"}
_ALIASES = {
    "apache2":"apache-2.0","bsd3":"bsd-3-clause","bsd2":"bsd-2-clause","gpl3":"gpl-3.0",
    "lgpl2.1":"lgpl-2.1","lgpl-2.1+":"lgpl-2.1-or-later","cc-by-nc-4":"cc-by-nc-4.0",
}
...
def _score(token: str) -> float:
    if token in _INCOMPAT_0_0: return 0.0
    if token in _COMPAT_1_0:   return 1.0
    if token in {"mpl-2.0","cddl-1.0","cddl-1.1"}: return 0.7
    if token in {"other","unknown",""}: return 0.2
    return 0.2
...
class LicenseMetric:
    ...
    def compute(self, ctx: Dict[str, Any]) -> MetricResult:
        ...
        if m:
            ...
            candidates = re.findall(r"(apache[-\s]?2\.0|mit|bsd[\s-]?(?:2|3)|lgpl[-\s]?2\.1(?:-or-later)?|gpl[-\s]?3|agpl[-\s]?3|mpl[-\s]?2\.0|cddl[-\s]?1\.?1?|cc[-\s]?by[-\s]?nc)", blob, re.I)
            if candidates:
                readme_lic = _norm(min((c.strip() for c in candidates), key=len))

        sources, token = [], ""
        if hf_lic and hf_lic not in {"other","unknown"}:
            token = hf_lic; sources.append("hf")
        if (not token) and readme_lic:
            token = readme_lic; sources.append("readme")
