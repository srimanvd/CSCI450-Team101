...
import os, re, tempfile, time, shutil
from typing import Dict, Any, Iterable
from .base import MetricResult

TRY_FILES = ("pytest.ini","conftest.py","tox.ini",".github/workflows","tests/","test/",".circleci/","azure-pipelines.yml")
...
def _pyproject_has(section: str, key_sub: str, root: str) -> bool:
    ...
    if not os.path.isfile(pp): return False
    try:
        import tomllib
...
def _safe_clone(url: str, max_seconds: int = 20) -> str | None:
    try:
        import git, threading
        d = tempfile.mkdtemp(prefix="repo_")
        ok = {"done": False, "path": d, "err": None}
        def _do():
            try:
...
class CodeQualityMetric:
    ...
    def compute(self, ctx: Dict[str, Any]) -> MetricResult:
        ...
        base = 0.0
        extras = {"repo_used": repo_url, "checks": {}}
        ...
        try:
            ...
            extras["checks"] = {
                "tests": has_tests, "ci": has_ci, "types": has_type,
                "lint": has_lint, "readme_code_blocks": readme_blocks,
