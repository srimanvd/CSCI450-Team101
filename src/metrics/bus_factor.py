...
import time, tempfile, shutil, datetime as dt
...
class BusFactorMetric:
    ...
    def compute(self, ctx: Dict[str, Any]) -> MetricResult:
        ...
        if not repo_url:
            return MetricResult(score=0.0, latency_ms=int((time.perf_counter()-t0)*1000), extras={"reason":"no_repo"})

        root = _safe_clone(repo_url)
        if not root:
            return MetricResult(score=0.0, latency_ms=int((time.perf_counter()-t0)*1000), extras={"reason":"clone_timeout"})
        ...
        try:
            ...
            # score: more contributors, less concentration → higher
            s_contrib = min(1.0, contrib/10.0)   # 10+ contributors saturates
            s_conc   = 1.0 - gini                # 1 = equal distribution
